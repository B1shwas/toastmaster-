...
import { ViewAgendaReportProps } from "@/lib/types/agendaReport";
import { CreateAgendaReport } from "./CreateAgendaReport";
    ...
    return (
        <div className="flex flex-col gap-y-4 text-white">
            <div className="flex justify-between items-center">
                ...
                >
                    {isEdit ?  <EyeIcon/> : <PenBoxIcon/>}
                </Button>
            </div>
            {
              isEdit ? <CreateAgendaReport canUserCreateIt={reportData} onEditSuccess={handleEditSuccess}/> : 
            <div className="flex flex-col gap-y-4 text-white">
                        ...
                        <div>
                            ...
                            <p className="text-2xl font-bold">
                                {isGrammarian
                                    ? (
                                          totalWordUsage /
                                          (report?.memberEvaluations?.length ||
                                              1)
                                      )?.toFixed(1)
                                    : (
                                          totalFillers /
                                          (report?.fillerWordCounts?.length ||
                                              1)
                                      )?.toFixed(1)}
                            </p>
                        </div>
                ...
                {isGrammarian && report?.grammarNotes && (
                    <div className="flex flex-col gap-y-2 border bg-neutral-800/50 border-neutral-700/50 rounded-xl p-4">
                        <h2 className="text-neutral-400 text-xs uppercase tracking-wider font-medium mb-2">
                            Grammar Notes
                        </h2>
                ...
                <div className="flex flex-col gap-y-2 border border-stone-600 bg-neutral-800/50 rounded-xl p-2">
                    <h2 className="text-neutral-400 text-xs uppercase tracking-wider font-medium mb-2 m-2">
                        Member Evaluations
                    </h2>
